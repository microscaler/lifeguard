...
**Root Cause:**
- `find_related()` is defined in the `FindRelated` trait with a specific signature: `fn find_related<R>() -> SelectQuery<R> where R: LifeModelTrait + Related<Self::Entity>`
...
**Recommended Approach:** Enhance `ModelTrait` to support `get_all_primary_key_values()` for composite keys, and use relationship metadata (via static registry) to map them to foreign key columns. This requires:
