...
use lifeguard::{LifeEntityName, LifeModelTrait, Related, RelationDef};
...
pub enum Relation {
    ...
    User,
    #[lifeguard(has_many_through = "TagEntity", through = "PostTagEntity")]
    Tags,
}
...
fn test_derive_relation_has_many_through() {
    ...
    assert_eq!(
        rel_def.rel_type,
        lifeguard::relation::def::RelationType::HasManyThrough
    );

    // Verify through_tbl is set
    assert!(
        rel_def.through_tbl.is_some(),
        "has_many_through should have through_tbl set"
    );
    ...
    assert!(
        rel_def.through_from_col.is_some(),
        "has_many_through should have through_from_col set"
    );
    match rel_def.through_from_col.as_ref().unwrap() {
        lifeguard::Identity::Unary(col) => {
            assert_eq!(
                col.to_string(),
                "post_id",
                "through_from_col should be 'post_id' for Post -> PostTags -> Tags"
            );
        }
    ...
    assert!(
        rel_def.through_to_col.is_some(),
        "has_many_through should have through_to_col set"
    );
    match rel_def.through_to_col.as_ref().unwrap() {
        lifeguard::Identity::Unary(col) => {
            assert_eq!(
                col.to_string(),
                "tag_id",
                "through_to_col should be 'tag_id' for Post -> PostTags -> Tags"
            );
        }
...
fn test_derive_relation_has_many_through_join_exprs() {
    ...
    let (first_join, second_join) = rel_def
        .join_on_exprs()
        .expect("join_on_exprs should succeed for has_many_through");
...
fn test_derive_relation_join_on_exprs_errors_on_non_has_many_through() {
    ...
    assert!(
        error_msg.contains("join_on_exprs() can only be called on HasManyThrough relationships")
    );
}
...
fn test_derive_relation_default_columns() {
    ...
    assert_ne!(
        to_col_name, "id",
        "to_col should be foreign key (post_id), not primary key (id)"
    );
    // The foreign key should be "post_id" (from "posts" table)
    assert_eq!(
        to_col_name, "post_id",
        "to_col should be post_id for Post has_many Comments"
    );
    ...
    assert_eq!(
        from_col_name, "id",
        "from_col should be primary key (id) for has_many"
    );
}
...
fn test_derive_relation_belongs_to_default_columns() {
    ...
    assert_ne!(
        from_col_name, "id",
        "from_col should be foreign key (author_id), not primary key (id)"
    );
    // The foreign key should be "author_id" (inferred from AuthorEntity)
    assert_eq!(
        from_col_name, "author_id",
        "from_col should be author_id for Article belongs_to Author"
    );
    ...
    assert_eq!(
        to_col_name, "id",
        "to_col should be primary key (id) in AuthorEntity for belongs_to"
    );
    ...
    fn infer_fk_name(entity_path: &str) -> String {
        ...
        let snake_case =
            if entity_name.contains('_') || entity_name.chars().all(|c| c.is_lowercase()) {
    ...
    assert_eq!(
        infer_fk_name("super::super::comments::Entity"),
        "comment_id"
    );
}
...
fn test_derive_relation_module_qualified_path() {
    ...
    // These should extract the module name, not produce "_id"
    assert_eq!(
        infer_fk_name("super::users::Entity"),
        "user_id",
...
fn test_derive_relation_self_referential() {
    ...
    assert_eq!(
        from_col_name, "parent_id",
        "from_col should be parent_id for Category belongs_to Category"
    );
    ...
    assert_eq!(
        to_col_name, "id",
        "to_col should be primary key (id) for Category belongs_to Category"
    );
...
mod self_referential_no_columns_test {
    use lifeguard::{LifeEntityName, LifeModelTrait, RelationDef};
    use lifeguard_derive::DeriveRelation;
