...
fn test_long_enum_name() {
    ...
    assert_eq!(
        enum_name,
        Some("very_long_enum_name_that_might_be_used_in_postgresql_schema".to_string())
    );
}
...
fn test_special_characters_in_column_type() {
    ...
    assert_eq!(
        def.column_type,
        Some("VARCHAR(255) COLLATE \"C\"".to_string())
    );
}
...
fn test_select_as_attribute() {
    ...
    assert_eq!(
        def_full_name.select_as,
        Some("CONCAT(first_name, ' ', last_name) AS full_name".to_string())
    );
...
fn test_select_as_all_columns_method() {
    ...
    assert_eq!(
        columns.len(),
        4,
        "Should have 4 columns: id, first_name, last_name, full_name"
    );
    ...
    assert!(
        found_full_name,
        "Should find full_name column with select_as"
    );
    assert!(
        found_id,
        "Should find at least one column without select_as"
    );
}
...
fn test_comment_attribute() {
    ...
    assert_eq!(
        def_email.comment,
        Some("Email address for authentication".to_string())
    );
...
fn test_comment_sql_generation() {
    ...
    assert_eq!(
        sql,
        Some("COMMENT ON COLUMN users.name IS 'User''s full name';".to_string())
    );
    ...
    assert_eq!(
        sql_schema,
        Some("COMMENT ON COLUMN public.users.name IS 'User''s full name';".to_string())
    );
...
fn test_comment_integration_with_life_model() {
    ...
    assert_eq!(
        name_sql,
        Some("COMMENT ON COLUMN test_comment_integration.name IS 'User''s full name';".to_string())
    );
...
fn test_comment_empty_string() {
    ...
    assert_eq!(
        sql,
        Some("COMMENT ON COLUMN test_empty_comment.name IS '';".to_string())
    );
}
...
fn test_comment_with_backslashes() {
    ...
    assert!(
        sql_str.contains("C:\\\\Users\\\\Documents"),
        "Backslashes should be escaped"
    );
    assert!(sql_str.contains("COMMENT ON COLUMN"));
    // Verify the escaped format is correct
    assert!(
        !sql_str.contains("C:\\Users"),
        "Should not have unescaped backslashes"
    );
}
...
fn test_comment_sql_with_special_table_name() {
    ...
    assert_eq!(
        sql1,
        Some("COMMENT ON COLUMN public.users.email IS 'Test comment';".to_string())
    );
    ...
    assert_eq!(
        sql2,
        Some("COMMENT ON COLUMN user_profiles.first_name IS 'Test comment';".to_string())
    );
}
...
fn test_multiple_select_as_expressions() {
    ...
    assert_eq!(
        regular_count, 2,
        "Should have 2 columns without select_as (id and regular)"
    );
}
