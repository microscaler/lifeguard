...
pub fn derive_life_model(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        // Entity registration for migration tool (Option B: Proc-Macro Registry)
        // Generate marker file in OUT_DIR for build script to collect
        // This allows the migration tool to discover all entities
        #[cfg(feature = "migration-registry")]
        {
            use std::fs;
            use std::env;
            use std::path::PathBuf;
            
            // Only generate marker if feature is enabled
            if let Ok(out_dir) = env::var("OUT_DIR") {
                let marker_dir = PathBuf::from(&out_dir).join("lifeguard_entities");
                let _ = fs::create_dir_all(&marker_dir);
                
                let marker_file = marker_dir.join(format!("{}.json", #table_name_lit));
                let marker = format!(
                    r#"{{
  "table_name": "{}",
  "struct_name": "{}",
  ...
  "file_path": "{}"
                ...
                let _ = fs::write(&marker_file, marker);
            }
        }
