...
/// - `FromRow` implementation (via separate `FromRow` derive macro)
...
pub fn derive_life_model(input: TokenStream) -> TokenStream {
    ...
    // 1. Generate Entity, Column, PrimaryKey first (by expand_derive_entity_model)
    // 2. Generate Model and FromRow (by expand_derive_model) 
    // 3. Generate EntityTrait implementation (by expand_derive_entity_model, after Model exists)
    //
    ...
    let expanded = quote! {
        ...
        // NOTE: LifeModelTrait implementation should be generated via DeriveLifeModelTrait
        // This can be applied via nested macro expansion: #[derive(DeriveLifeModelTrait)]
        // But we can't do nested expansion here, so users must apply it separately
        ...
        // Implement Iden for Column enum
        ...
        // STEP 2: Generate Model struct (like SeaORM's expand_derive_model)
        // Model struct (immutable row representation)
        #[derive(Debug, Clone)]
