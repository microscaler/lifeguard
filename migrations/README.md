...
2. **Generated SQL** (`generated/inventory/`): SQL migrations automatically generated from entities
   ...
   - **Delta mode:** all `*_generated_from_entities.sql` files in the service directory are read in **timestamp order** and merged (full `CREATE TABLE` snapshots plus later delta-only sections with `ALTER TABLE ... ADD COLUMN`). The effective schema is compared to the current entity output; unchanged tables produce **no** repeated `CREATE TABLE`. New columns become `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, and new index lines use `CREATE INDEX IF NOT EXISTS` where applicable. If nothing changed, **no new file** is written. The latest file must not be the only baseline when it is ALTER-only—older files in the same folder supply the `CREATE TABLE` definitions. Each migration must use `-- Table: <name>` section headers (as emitted by the generator).
   - Avoid keeping many **byte-identical** timestamp copies in the same folder — they confuse humans and inflate `apply_order.txt`; keep one current baseline (or rely on version control history) before re-running generation.
...
```bash
# From the examples/entities directory (or repo root with --manifest-path examples/entities/Cargo.toml)
