...
2. **Generated SQL** (`generated/inventory/`): SQL migrations automatically generated from entities
   ...
   - **Delta mode:** if the service directory already contains a `*_generated_from_entities.sql`, the next run compares that **latest** file (by timestamp prefix) to the current entity output. Unchanged tables produce **no** repeated `CREATE TABLE`; new columns become `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, and new index lines use `CREATE INDEX IF NOT EXISTS` where applicable. If nothing changed, **no new file** is written. A second check compares each `-- Table: <name>` section to freshly generated SQL (ignoring top-of-file `-- Version:` / `-- Generated:` lines), so a new timestamp alone never forces a duplicate file. The baseline file must use `-- Table: <name>` section headers (as emitted by the generator).
