...
│   └── (empty - will be populated by entity-driven generation)
│
├── README.md          # This file
...
1. **Original SQL** (`original/`): The existing SQL migrations that were manually created
2. **Entity Definitions**: Lifeguard entities (using `#[derive(LifeModel)]`) that represent the desired database schema
3. **Generated SQL** (`generated/`): SQL migrations automatically generated by diffing current entities vs previous state

### Migration Process

1. **Build Entities**: Create Lifeguard entities that match the SQL in `original/`
   - Start with Chart of Accounts entities
   - Then Accounts, Journal Entries, etc.
   - Ensure all fields, types, constraints, and indexes match
2. **Generate Migrations**: Run `lifeguard-migrate generate --from-entities` to create SQL in `generated/`
3. **Compare & Validate**: Diff `original/` vs `generated/` to ensure they produce equivalent SQL
