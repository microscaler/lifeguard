...
use lifeguard::{MayPostgresExecutor, SelectQuery, FromRow};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\nExample 4: Pagination with count");
    let mut paginator = UserModel::find()
        .filter(Expr::col("age").gte(18))
    ...
    let users_with_email = UserModel::find()
        .filter(Expr::col("email").is_not_null())
        .all(&executor)?;
    println!("  Found {} users with email addresses", users_with_email.len());
