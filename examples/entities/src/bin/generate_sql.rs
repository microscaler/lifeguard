...
use example_entities::entity_registry;
use std::fs;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (service_path, entities) in &sql_by_service {
        ...
        println!("✅ Generated SQL migration for {}: {}", service_path, output_file.display());
    }
    
    println!("✅ Success - Generated {} migration file(s)", sql_by_service.len());
