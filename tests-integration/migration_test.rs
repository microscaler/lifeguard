...
use lifeguard::{
    migration::{
        clear_registry, initialize_state_table, register_migration, release_migration_lock,
        unregister_migration, Migration, MigrationRecord, Migrator, SchemaManager,
    },
    test_helpers::TestDatabase,
    LifeError, LifeExecutor,
};
...
fn test_migration_lifecycle() {
    ...
    initialize_state_table(&executor).expect("Failed to initialize migration state table");
    ...
    let _ = release_migration_lock(&executor);
    cleanup_all_test_artifacts(&executor).expect("Failed to clean up all test artifacts");
    ...
    cleanup_test_artifacts(&executor, version).expect("Failed to clean up test artifacts");
    ...
    let lock_held = is_migration_lock_held(&executor).expect("Failed to check lock status");
    if lock_held {
    ...
    register_migration(Box::new(migration_for_registry)).expect("Failed to register migration");
    ...
    verify_database_schema(&executor, version).expect("Failed to verify database schema");
...
fn test_migration_state_table_creation() {
    ...
    let rows = executor
        .query_all(columns_sql, &[])
        .expect("Failed to query columns");
    assert!(
        rows.len() >= 6,
        "State table should have at least 6 columns"
    );
