...
/// Legacy compatibility: MigrationLock struct
///
/// This is kept for backward compatibility but is deprecated.
/// Use `MigrationLockGuard::new()` or `acquire_migration_lock()` directly instead.
#[deprecated(note = "Use MigrationLockGuard::new() or acquire_migration_lock() instead")]
pub struct MigrationLock;

impl MigrationLock {
    /// Acquire a migration lock (legacy method)
    ///
    /// # Deprecated
    ///
    /// This method is deprecated. Use `MigrationLockGuard::new()` instead,
    /// which works with executor references and doesn't require ownership.
    #[deprecated(note = "Use MigrationLockGuard::new() instead")]
    pub fn acquire(
        executor: Box<dyn LifeExecutor>,
        timeout_seconds: Option<u64>,
    ) -> Result<LockGuard, MigrationError> {
        // For backward compatibility, we need to return LockGuard
        // But LockGuard is also deprecated, so this is a transitional state
        Err(MigrationError::InvalidFormat(
            "MigrationLock::acquire() is deprecated. Use MigrationLockGuard::new() instead."
                .to_string()
        ))
    }
}

/// Legacy compatibility: LockGuard struct
///
/// This is kept for backward compatibility but is deprecated.
/// Use `MigrationLockGuard` instead.
#[deprecated(note = "Use MigrationLockGuard instead")]
...
impl LockGuard {
    #[deprecated(note = "Use MigrationLockGuard::executor() instead")]
