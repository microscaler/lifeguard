...
static MIGRATION_REGISTRY: LazyLock<Mutex<HashMap<i64, Box<dyn Migration + Send + Sync>>>> =
...
pub fn register_migration(
    migration: Box<dyn Migration + Send + Sync>,
) -> Result<(), MigrationError> {
    let version = migration.version();
    let name = migration.name().to_string();

    let mut registry = MIGRATION_REGISTRY.lock().map_err(|e| {
        MigrationError::InvalidFormat(format!("Failed to lock migration registry: {e}"))
    })?;
...
pub fn get_migration(
    version: i64,
) -> Result<Option<Box<dyn Migration + Send + Sync>>, MigrationError> {
    let registry = MIGRATION_REGISTRY.lock().map_err(|e| {
        MigrationError::InvalidFormat(format!("Failed to lock migration registry: {e}"))
    })?;
...
mod tests {
    ...
    fn test_register_migration_duplicate_different_name_still_fails() {
        ...
        match result {
            Ok(()) => {
                ...
                assert!(
                    result2.is_err(),
                    "Should fail even with different name on retry"
                );
                #[allow(clippy::unwrap_used)] // Test code - unwrap_err is acceptable
                match result2.unwrap_err() {
                    MigrationError::AlreadyRegistered {
                        ...
                        assert_eq!(
                            n, "third_migration",
                            "Error should contain third migration name"
                        );
                    }
                    #[allow(clippy::panic)] // Test code - panic is acceptable
                    other => panic!("Expected AlreadyRegistered, got {other:?}"),
            ...
            Err(error) => {
                match error {
                    MigrationError::AlreadyRegistered {
                        ...
                        assert_eq!(
                            n, "second_migration",
                            "Error should contain second migration name"
                        );
                    }
                    #[allow(clippy::panic)] // Test code - panic is acceptable
                    other => panic!("Expected AlreadyRegistered, got {other:?}"),
    ...
    fn test_register_multiple_different_versions() {
        ...
        if !is_registered(version1).expect("Failed to check") {
            ...
            register_migration(Box::new(migration1_retry))
                .expect("Should register first migration on retry");
        }
        assert!(
            is_registered(version1).expect("Failed to check"),
        ...
        if !is_registered(version2).expect("Failed to check") {
            ...
            register_migration(Box::new(migration2_retry))
                .expect("Should register second migration on retry");
        }
        assert!(
            is_registered(version2).expect("Failed to check"),
        ...
        if !is_registered(version3).expect("Failed to check") {
            ...
            register_migration(Box::new(migration3_retry))
                .expect("Should register third migration on retry");
        }
        assert!(
            is_registered(version3).expect("Failed to check"),
        ...
        assert!(
            all_versions.contains(&version1),
            "Should contain first migration version"
        );
        assert!(
            all_versions.contains(&version2),
            "Should contain second migration version"
        );
        assert!(
            all_versions.contains(&version3),
            "Should contain third migration version"
        );

        // Verify they appear in sorted order (relative to each other)
        #[allow(clippy::unwrap_used)]
        // Test code - unwrap is acceptable (we just asserted these exist)
        ...
        assert!(
            pos1 < pos2 && pos2 < pos3,
            "Versions should be in sorted order"
        );
    }
    ...
    fn test_register_after_unregister() {
        ...
        // Register
        let migration1 = TestMigration::new(version, name);
        register_migration(Box::new(migration1)).expect("Should register successfully");
        ...
        let removed = unregister_migration(version).expect("Should unregister successfully");
        ...
        // Test code - explicit boolean comparison is acceptable for clarity
        if removed == false {
            ...
            if still_registered {
                ...
                // Try again
                let removed_retry =
                    unregister_migration(version).expect("Should unregister successfully on retry");
                assert!(
                    removed_retry,
                    "Should return true when migration was removed on retry"
                );
            }
            ...
            assert!(
                !is_registered(version).expect("Failed to check"),
        ...
        assert!(
            result.is_ok(),
            "Should be able to register after unregister"
        );
    }
    ...
    fn test_already_registered_error_message() {
        ...
        assert!(
            is_reg,
            "Migration must be registered before testing duplicate registration"
        );
        ...
        assert!(
            error_msg.contains(&version.to_string()),
