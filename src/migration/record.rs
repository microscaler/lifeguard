...
impl MigrationRecord {
    ...
    pub fn from_row(row: &may_postgres::Row) -> Result<Self, crate::LifeError> {
        ...
        let applied_at = {
            ...
            if let Ok(naive) =
                chrono::NaiveDateTime::parse_from_str(&applied_at_str, "%Y-%m-%d %H:%M:%S%.f")
            {
                naive.and_utc()
                ...
                chrono::NaiveDateTime::parse_from_str(&applied_at_str, "%Y-%m-%d %H:%M:%S")
            {
                ...
                chrono::NaiveDateTime::parse_from_str(&applied_at_str, "%Y-%m-%dT%H:%M:%S%.f")
            {
                naive.and_utc()
            } else if let Ok(naive) =
                chrono::NaiveDateTime::parse_from_str(&applied_at_str, "%Y-%m-%dT%H:%M:%S")
            {
                naive.and_utc()
            } else {
