    ...
    Ex: LifeExecutor,
{
    ...
    if rel_def.rel_type == RelationType::HasManyThrough {
        ...
        let through_from_col = rel_def.through_from_col.as_ref().ok_or_else(|| {
            LifeError::Other("HasManyThrough relationship must have through_from_col set".to_string())
        })?;
        ...
        if fk_arity == 1 {
            ...
            if !pk_values.is_empty() {
                let through_from_col_name = through_from_col.iter().next().ok_or_else(|| {
                    LifeError::Other("through_from_col must have at least one column".to_string())
                })?.to_string();
                let through_to_col_name = through_to_col.iter().next().ok_or_else(|| {
                    LifeError::Other("through_to_col must have at least one column".to_string())
                })?.to_string();
                let through_tbl_str = extract_table_name(through_tbl);
                let target_pk_col_name = rel_def.to_col.iter().next().ok_or_else(|| {
                ...
                let pk_values_str: Vec<String> = pk_values.iter().map(value_to_sql_string).collect();
                ...
                let filter_expr = format!("{target_tbl_str}.{target_pk_col_name} IN {subquery_sql}");
            ...
            let through_to_col_names: Vec<String> = through_to_col.iter().map(ToString::to_string).collect();
            let through_tbl_str = extract_table_name(through_tbl);
            ...
            let target_tbl_str = extract_table_name(&rel_def.to_tbl);
            let target_pk_col_names: Vec<String> = rel_def.to_col.iter().map(ToString::to_string).collect();
            ...
            let filter_expr = format!(
                "({target_tbl_str}.{target_pk_col_names_str}) IN {subquery_sql}"
            );
            query = query.filter(Expr::cust(filter_expr));
        }
    } else {
    ...
    if fk_arity == 1 {
        ...
        if !pk_values.is_empty() {
            ...
            let fk_col = rel_def.to_col.iter().next().ok_or_else(|| {
                LifeError::Other("to_col must have at least one column for relationship".to_string())
            })?;
    ...
    if rel_def.rel_type == RelationType::HasManyThrough {
        ...
        let through_tbl = rel_def.through_tbl.as_ref().ok_or_else(|| {
            LifeError::Other("HasManyThrough must have through_tbl".to_string())
        })?;
        let through_from_col = rel_def.through_from_col.as_ref().ok_or_else(|| {
        ...
        if !target_pk_values.is_empty() {
            ...
            let through_from_col_names: Vec<String> = through_from_col.iter().map(ToString::to_string).collect();
            let through_to_col_names: Vec<String> = through_to_col.iter().map(ToString::to_string).collect();
...
mod tests {
    ...
    fn test_value_to_sql_string_integers() {
        ...
        assert_eq!(value_to_sql_string(&Value::BigInt(Some(1_234_567_890))), "1234567890");
    ...
    fn test_value_to_sql_string_unsigned_integers() {
        ...
        assert_eq!(value_to_sql_string(&Value::BigUnsigned(Some(999_999))), "999999");
    ...
    fn test_find_linked_query_building() {
        ...
        impl sea_query::Iden for CommentEntity {
            fn unquoted(&self) -> &'static str { "comments" }
        }
    ...
    fn test_single_key_eager_loading_parameter_binding() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let fk_col = rel_def.to_col.iter().next().ok_or_else(|| {
            LifeError::Other("to_col must have at least one column for belongs_to relationship".to_string())
        })?;
        let fk_col_str = fk_col.to_string();
        let fk_col_iden = sea_query::DynIden::from(fk_col_str);
    ...
    fn test_load_related_has_many_through_uses_subquery() {
        ...
        impl sea_query::Iden for TagEntity {
            fn unquoted(&self) -> &'static str { "tags" }
        }
    ...
    fn test_load_related_has_many_through_grouping_positive() {
        ...
        impl ModelTrait for UserModel {
            ...
            fn set(&mut self, col: UserColumn, val: sea_query::Value) -> Result<(), crate::model::ModelError> { 
                match col {
                    UserColumn::Id => {
        ...
        impl ModelTrait for TagModel {
            ...
            fn set(&mut self, col: TagColumn, val: sea_query::Value) -> Result<(), crate::model::ModelError> { 
                match col {
                    TagColumn::Id => {
                        if let sea_query::Value::Int(Some(v)) = val {
                            self.id = v;
