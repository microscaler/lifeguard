...
use sea_query::{Condition, Expr, ExprTrait, Value};
...
fn value_to_sql_string(value: &Value) -> String {
    match value {
        // Null values - all return "NULL" in SQL
        #[allow(clippy::match_same_arms)] // All null variants correctly return "NULL"
        Value::Bool(None)
        | Value::TinyInt(None)
        | Value::SmallInt(None)
        | Value::Int(None)
...
/// #     fn get(&self, col: <User as lifeguard::LifeModelTrait>::Column) -> sea_query::Value { todo!() }
...
/// - `entities` is empty and a primary key is accessed
/// - A `HasManyThrough` relationship is used without a `through_tbl` being set
#[allow(clippy::too_many_lines)] // Complex relationship loading logic requires many lines
    ...
    Ex: LifeExecutor,
{
    ...
    if rel_def.rel_type == RelationType::HasManyThrough {
        ...
        let through_tbl = rel_def.through_tbl.as_ref().ok_or_else(|| {
            LifeError::Other("HasManyThrough relationship must have through_tbl set".to_string())
        })?;
        ...
        let through_to_col = rel_def.through_to_col.as_ref().ok_or_else(|| {
            LifeError::Other("HasManyThrough relationship must have through_to_col set".to_string())
        })?;
        ...
        if fk_arity == 1 {
            ...
            if !pk_values.is_empty() {
                let through_from_col_name = through_from_col
                    .iter()
                    .next()
                    .ok_or_else(|| {
                        LifeError::Other(
                            "through_from_col must have at least one column".to_string(),
                        )
                    })?
                    .to_string();
                let through_to_col_name = through_to_col
                    .iter()
                    .next()
                    .ok_or_else(|| {
                        LifeError::Other("through_to_col must have at least one column".to_string())
                    })?
                    .to_string();
                let through_tbl_str = extract_table_name(through_tbl);
                let target_pk_col_name = rel_def
                    .to_col
                    .iter()
                    .next()
                    .ok_or_else(|| {
                        LifeError::Other("to_col must have at least one column".to_string())
                    })?
                    .to_string();
                ...
                let pk_values_str: Vec<String> =
                    pk_values.iter().map(value_to_sql_string).collect();
                let pk_values_joined = pk_values_str.join(", ");

                let subquery_sql = format!(
                    "(SELECT {through_tbl_str}.{through_to_col_name} FROM {through_tbl_str} WHERE {through_tbl_str}.{through_from_col_name} IN ({pk_values_joined}))"
                );

                // Filter target table: target.pk IN (subquery)
                // Use Expr::cust() to build the full condition with embedded subquery
                let filter_expr =
                    format!("{target_tbl_str}.{target_pk_col_name} IN {subquery_sql}");
            ...
            for pk_vals in &unique_pk_values {
                ...
                for (through_from_col_item, pk_val) in through_from_col.iter().zip(pk_vals.iter()) {
                    ...
                    let col_expr =
                        format!("{through_tbl_str}.{through_from_col_name} = {sql_value}");
            ...
            let through_to_col_names: Vec<String> =
                through_to_col.iter().map(ToString::to_string).collect();
            let through_tbl_str = extract_table_name(through_tbl);
            ...
            for pk_vals in &unique_pk_values {
                ...
                for (through_from_col_item, pk_val) in through_from_col.iter().zip(pk_vals.iter()) {
                    ...
                    condition_parts.push(format!(
                        "{through_tbl_str}.{through_from_col_name} = {sql_value}"
                    ));
                }
            ...
            let subquery_sql = format!(
                "(SELECT {through_to_col_names_str} FROM {through_tbl_str} WHERE {subquery_where})"
            );
            ...
            let target_tbl_str = extract_table_name(&rel_def.to_tbl);
            let target_pk_col_names: Vec<String> =
                rel_def.to_col.iter().map(ToString::to_string).collect();
            ...
            let filter_expr =
                format!("({target_tbl_str}.{target_pk_col_names_str}) IN {subquery_sql}");
            query = query.filter(Expr::cust(filter_expr));
        }
    } else {
        ...
        if fk_arity == 1 {
            ...
            if !pk_values.is_empty() {
                ...
                let fk_col = rel_def.to_col.iter().next().ok_or_else(|| {
                    LifeError::Other(
                        "to_col must have at least one column for relationship".to_string(),
                    )
                })?;
    ...
    if rel_def.rel_type == RelationType::HasManyThrough {
        ...
        let through_tbl = rel_def
            .through_tbl
            .as_ref()
            .ok_or_else(|| LifeError::Other("HasManyThrough must have through_tbl".to_string()))?;
        let through_from_col = rel_def.through_from_col.as_ref().ok_or_else(|| {
            LifeError::Other("HasManyThrough must have through_from_col".to_string())
        })?;
        let through_to_col = rel_def.through_to_col.as_ref().ok_or_else(|| {
            LifeError::Other("HasManyThrough must have through_to_col".to_string())
        })?;
        ...
        for related in &related_entities {
            ...
            for target_pk_col in &rel_def.to_col {
        ...
        if !target_pk_values.is_empty() {
            ...
            let through_from_col_names: Vec<String> =
                through_from_col.iter().map(ToString::to_string).collect();
            let through_to_col_names: Vec<String> =
                through_to_col.iter().map(ToString::to_string).collect();

            // Build WHERE conditions for source PKs
            let source_pk_values_str: Vec<String> = unique_pk_values
                .iter()
                .map(|vals| {
                    if vals.len() == 1 {
                        ...
                        let vals_str: Vec<String> = vals.iter().map(value_to_sql_string).collect();
            ...
            let target_pk_values_str: Vec<String> = target_pk_values
                .iter()
                .map(|vals| {
                    if vals.len() == 1 {
                        value_to_sql_string(&vals[0])
            ...
            let select_cols =
                if through_from_col_names.len() == 1 && through_to_col_names.len() == 1 {
            ...
            let sql = format!(
                "SELECT {select_cols} FROM {through_tbl_str} WHERE {from_condition} AND {to_condition}"
            );
            ...
            for row in mapping_rows {
                ...
                let source_pk_key: String = source_pk_vals
                    .iter()
                    .map(|v| format!("{v:?}"))
                    .collect::<Vec<_>>()
                    .join("|");

                let target_pk_key: String = target_pk_vals
                    .iter()
                    .map(|v| format!("{v:?}"))
                    .collect::<Vec<_>>()
                    .join("|");
        ...
        'outer: for related in related_entities {
            ...
            for fk_col in &rel_def.to_col {
                ...
                if let Some(fk_value) = related.get_by_column_name(&fk_col_str) {
                    ...
                    {
                        use std::fmt::Write;
                        write!(fk_key, "{fk_value:?}").unwrap_or_default();
                    }
                } else {
            ...
            if let Some(matching_pk_key) = pk_to_values.keys().find(|pk_key| pk_key == &&fk_key) {
                if let Some(related_vec) = result.get_mut(matching_pk_key) {
                    related_vec.push(related);
                } else {
                    return Err(LifeError::Other(format!(
                        "Internal error: matching PK key '{matching_pk_key}' not found in result map"
                    )));
                }
            } else {
                // This shouldn't happen if the query was built correctly, but handle gracefully
...
mod tests {
    ...
    fn test_value_to_sql_string_integers() {
        ...
        assert_eq!(
            value_to_sql_string(&Value::BigInt(Some(1_234_567_890))),
            "1234567890"
        );
    ...
    fn test_value_to_sql_string_unsigned_integers() {
        ...
        assert_eq!(
            value_to_sql_string(&Value::BigUnsigned(Some(999_999))),
            "999999"
        );
        assert_eq!(value_to_sql_string(&Value::TinyUnsigned(Some(255))), "255");
        assert_eq!(
            value_to_sql_string(&Value::SmallUnsigned(Some(65535))),
            "65535"
        );
    }
    ...
    fn test_value_to_sql_string_floats() {
        ...
        assert_eq!(
            value_to_sql_string(&Value::Double(Some(2.71828))),
            "2.71828"
        );
    ...
    fn test_value_to_sql_string_strings() {
        ...
        assert_eq!(
            value_to_sql_string(&Value::String(Some("hello".to_string()))),
            "'hello'"
        );
        assert_eq!(
            value_to_sql_string(&Value::String(Some("world".to_string()))),
            "'world'"
        );
        // Test string with single quotes (should be escaped)
        assert_eq!(
            value_to_sql_string(&Value::String(Some("it's".to_string()))),
            "'it''s'"
        );
        assert_eq!(
            value_to_sql_string(&Value::String(Some("don't".to_string()))),
            "'don''t'"
        );
    }
    ...
    fn test_composite_key_condition_building() {
        ...
        assert!(
            !sql.contains("Int(Some(42))"),
            "SQL should NOT contain Debug output 'Int(Some(42))'"
        );
        assert!(
            !sql.contains("Some(42)"),
            "SQL should NOT contain Debug output 'Some(42)'"
        );
    }
    ...
    fn test_composite_key_condition_building_multiple_values() {
        ...
        for pk_vals in &pk_combinations {
            let mut and_condition = Condition::all();

            // Simulate the loop in load_related for composite keys
            let col_names = ["user_id", "tenant_id"];
        ...
        assert!(
            !sql.contains("Int(Some"),
            "SQL should NOT contain Debug output 'Int(Some'"
        );
        assert!(
            !sql.contains("Some("),
            "SQL should NOT contain Debug output 'Some('"
        );
    }
    ...
    fn test_composite_key_condition_building_with_strings() {
        ...
        assert!(
            sql.contains("'user123'"),
            "SQL should contain quoted string 'user123'"
        );
        assert!(
            !sql.contains("String(Some"),
            "SQL should NOT contain Debug output 'String(Some'"
        );
    }
    ...
    fn test_load_related_empty_entities() {
        ...
        impl crate::model::ModelTrait for TestModel {
            ...
            #[allow(clippy::todo)] // Test code - todo!() is acceptable for unimplemented test helpers
            fn get(&self, _col: TestColumn) -> sea_query::Value {
                todo!()
            }
            #[allow(clippy::todo)] // Test code - todo!() is acceptable for unimplemented test helpers
            fn set(
        ...
        where
            ...
            R::Model: ModelTrait + crate::query::traits::FromRow,
        {
            load_related(entities, executor)
        }
    ...
    fn test_find_linked_query_building() {
        ...
        impl ModelTrait for PostModel {
            ...
            // Parameters are part of trait interface - unused is expected
            fn set(
                &mut self,
                _col: PostColumn,
                _val: sea_query::Value,
        ...
        impl Related<CommentEntity> for PostEntity {
            fn to() -> RelationDef {
                RelationDef {
                    rel_type: RelationType::HasMany,
                    from_tbl: sea_query::TableRef::Table(
                        TableName(None, "posts".into_iden()),
                        None,
                    ),
                    to_tbl: sea_query::TableRef::Table(
                        TableName(None, "comments".into_iden()),
                        None,
                    ),
                    from_col: Identity::Unary("id".into()),
        ...
        impl crate::model::ModelTrait for TestModel {
            ...
            fn get_by_column_name(&self, column_name: &str) -> Option<sea_query::Value> {
                match column_name {
                    ...
                    "tenant_id" => Some(sea_query::Value::Int(Some(self.tenant_id))),
                    "user_id" => Some(sea_query::Value::Int(Some(self.user_id))),
                    _ => None, // Second FK column missing
                }
            }
        }
    ...
    #[test]
    fn test_single_key_eager_loading_parameter_binding() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let fk_col = rel_def.to_col.iter().next().ok_or_else(|| {
            LifeError::Other(
                "to_col must have at least one column for belongs_to relationship".to_string(),
            )
        })?;
        let fk_col_str = fk_col.to_string();
        let fk_col_iden = sea_query::DynIden::from(fk_col_str);
        ...
        assert_eq!(
            values_vec.len(),
            3,
            "Should have 3 parameter values bound for IN clause"
        );
        ...
        assert_eq!(
            placeholder_count, 3,
            "SQL should contain 3 parameter placeholders"
        );
        ...
        impl Related<TagEntity> for UserEntity {
            fn to() -> RelationDef {
                RelationDef {
                    ...
                    through_tbl: Some(sea_query::TableRef::Table(
                        TableName(None, "user_tags".into_iden()),
                        None,
                    )),
    ...
    fn test_load_related_has_many_through_grouping_positive() {
        ...
        impl ModelTrait for UserModel {
            ...
            fn set(
                ...
                match col {
                    UserColumn::Id => {
                        if let sea_query::Value::Int(Some(v)) = val {
                            self.id = v;
                        }
        ...
        impl ModelTrait for TagModel {
            ...
            fn get(&self, col: TagColumn) -> sea_query::Value {
                match col {
    ...
    fn test_load_related_has_many_through_grouping_negative() {
        ...
        #[allow(clippy::items_after_statements)] // Test code - function definition after statement is acceptable
        #[allow(clippy::used_underscore_binding)] // Test code - underscore prefix is intentional for unused parameter
        fn _test_has_many_through_empty_results<
            M: ModelTrait,
            R: LifeModelTrait,
            Ex: LifeExecutor,
        >(
