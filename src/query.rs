...
pub trait LifeModelTrait: LifeEntityName {
    ...
    ///
    /// This is generated by the `LifeModel` derive macro as an enum with variants
    /// for each field in the model.
    type Column: sea_query::Iden + Copy + Clone;
    ...
    E: LifeModelTrait,
{
        ...
        E::Model: FromRow,
    {
        ...
        for value in values.iter() {
            match value {
                ...
                sea_query::Value::TinyInt(Some(_))
    ...
    /// ```
    pub fn paginate<'e, Ex: LifeExecutor>(
        self,
        executor: &'e Ex,
        page_size: usize,
    ) -> Paginator<'e, E, Ex>
    ...
    pub fn count<Ex: LifeExecutor>(&self, executor: &Ex) -> Result<usize, LifeError> {
        ...
        for value in values.iter() {
            match value {
                ...
                _ => {
                    return Err(LifeError::Other(format!(
                        "Unsupported value type in COUNT query"
                    )))
                }
            }
        }
    ...
    /// let page_1 = paginator.fetch_page(1)?;
    /// ```
    pub fn paginate_and_count<'e, Ex: LifeExecutor>(
        self,
        executor: &'e Ex,
        page_size: usize,
    ) -> PaginatorWithCount<'e, E, Ex>
    where
        E::Model: FromRow,
    {
...
mod tests {
    ...
    fn test_find_one_no_results() {
        ...
        match result {
            ...
            Err(e) => panic!(
                "find_one should return Ok(None) for 'no rows' errors, got: {:?}",
                e
            ),
        }
    }
