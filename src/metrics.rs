...
impl LifeguardMetrics {
    pub fn init() -> Self {
        ...
        let queries_total = meter
            .u64_counter("lifeguard_queries_total")
            .with_description("Total queries executed")
            .build();

        let query_duration = meter
            .f64_histogram("lifeguard_query_duration_seconds")
            .with_description("Query duration")
            .build();

        let coroutine_wait_duration = meter
            .f64_histogram("lifeguard_coroutine_wait_seconds")
            .with_description("Coroutine wait time for query")
            .build();
        ...
        meter
            .u64_observable_gauge("lifeguard_pool_queue_depth")
            .with_description("DB pool queue depth")
            .with_callback(move |observer| {
                observer.observe(depth_clone.load(Ordering::Relaxed) as u64, &[]);
            });

        Self {
            exporter: exporter.expect("REASON"),
            queries_total,
            query_duration,
