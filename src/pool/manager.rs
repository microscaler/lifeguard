...
impl DbPoolManager {
    ...
    pub fn from_config(config: &DatabaseConfig) -> Result<Self, DbErr> {
        Self::new_with_params(&config.url, config.max_connections)
    }

    /// Shorthand using default config
    pub fn new() -> Result<Self, DbErr> {
        Self::from_config(&DatabaseConfig::load())
    }
...
mod tests {
    ...
    async fn test_macro_and_async_work_together() -> Result<(), DbErr> {
        let pool = DbPoolManager::from_config(&DatabaseConfig::default())?;
    ...
    async fn test_execute_and_unprepared() -> Result<(), sea_orm::DbErr> {
        let db = DbPoolManager::from_config(&DatabaseConfig::default())?;
    ...
    async fn test_seed_test_macro() -> Result<(), sea_orm::DbErr> {
        ...
        seed_test!(users, "(id INT, name TEXT, active BOOLEAN)", [
