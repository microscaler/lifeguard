...
2. **`test_derive_relation_belongs_to_default_columns()`**: Comprehensive test for `belongs_to` default inference
   - Creates a separate test module with `ArticleEntity` and `AuthorEntity`
   - Tests actual `belongs_to` relationship without `from`/`to` attributes
   - Verifies that `from_col` is `author_id` (foreign key), not `id` (primary key)
   - Verifies that `to_col` is `id` (primary key in target table)
   - Also includes FK name inference logic tests for completeness
...
test test_derive_relation_composite_key_parsing ... ok
test test_derive_relation_multiple_relationships ... ok
test test_derive_relation_generates_related_impls ... ok
test test_derive_relation_with_metadata ... ok
test test_derive_relation_has_one ... ok
test test_derive_relation_path_qualified_columns ... ok
