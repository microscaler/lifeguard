...
The crate exposes **parameterized SQL helpers** (`execute_statement`, `find_by_statement`, `find_all_by_statement`, and `query_value`) from [`src/raw_sql.rs`](../../../src/raw_sql.rs), re-exported at the crate root in [`src/lib.rs`](../../../src/lib.rs). Lifeguard intentionally does not expose an `execute_unprepared` application helper. Schema changes belong in entities and migrations; dynamic values must be bound.
...
4. **RLS is executor/transaction scoped.** Parameterized SQL that touches a protected table must run on the executor supplied by `with_session_transaction`, exactly like ORM work. The runtime database role remains subject to forced RLS.
...
Raw SQL concentrates **injection** and **schema drift** risk on the caller. [`SECURITY_PROMPT.md`](../../../SECURITY_PROMPT.md) treats query construction as an audit theme; prefer typed, generated, or builder-backed SQL. Do not emulate the removed helper with `executor.execute(sql, &[])` for dynamic application or schema work.
