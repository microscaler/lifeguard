...
- [x] **PRD follow-on closure (this document):** §5.7a **`compare-schema`** includes access-method drift, **T1**-style normalized index statement text when names match, index-only-in-DB/migration reporting, and **T2b (partial)** catalog **non-default btree opclass** vs type default (`fetch_live_btree_index_key_opclasses`); **T4** opt-in **`#[require_index_coverage]`** on **`LifeModel`**; §7.7 optional struct scope-list codegen **closed** (use **`#[scope_bundle]`**); §9.7 mapping discipline = **G6**; optional **T2b** follow-ons / **T3** remain in **[DESIGN_INDEX_COMPARE_ROADMAP.md](./DESIGN_INDEX_COMPARE_ROADMAP.md)** and **[ROADMAP.md](../../ROADMAP.md)**.
...
| **4** | Phase C | **Related-side scope (opt-in):** **`FindRelated::find_related_scoped`** — same as `find_related()?.scope(…)`; integration test `test_find_related_scoped_matches_chained_scope`. **Inherited parent scopes** (merge parent `SelectQuery` into `find_related` SQL) + loaders — **spike completed:** **no new implicit-merge API**; pattern **A + D** in **[DESIGN_INHERITED_PARENT_SCOPES_SPIKE.md](./DESIGN_INHERITED_PARENT_SCOPES_SPIKE.md)** (B/C deferred). |

### 0.4 Expanded: next three follow-on items (teaching + planning)

Rows **1**, **2**, and **4** from the table above are expanded in one place — **usage today vs future**, **G6 checklist**, **example surface** (`examples/find_related_scope_example.rs` in the crate root), and **why inherited parent + loader merge is highest risk** — in **[PRD_FOLLOWON_NEXT_THREE.md](./PRD_FOLLOWON_NEXT_THREE.md)**. **G6** is also summarized in **[DEVELOPMENT.md](../DEVELOPMENT.md)**. **Inherited parent + loaders:** **[DESIGN_INHERITED_PARENT_SCOPES_SPIKE.md](./DESIGN_INHERITED_PARENT_SCOPES_SPIKE.md)** (completed spike — recommendation **A + D**). **Index comparison §5.7a stretch:** **[DESIGN_INDEX_COMPARE_ROADMAP.md](./DESIGN_INDEX_COMPARE_ROADMAP.md)**.
...
- **Library:** `lifeguard_migrate::schema_infer::{infer_schema_rust, InferOptions}` — introspects `information_schema`, maps common PostgreSQL types to Rust types conservatively, emits `#[derive(LifeModel, LifeRecord)]` structs with `#[primary_key]` on **each** primary-key column (including **composite** PKs — multiple `#[primary_key]` attributes, matching `lifeguard-derive`); unsupported types are **omitted** with `// OMITTED:` lines (SI-2). **Secondary btree indexes** (non-primary `pg_index` rows, `pg_am.amname = 'btree'`) are introspected via `pg_index` / `pg_get_indexdef` / `pg_opclass` / `pg_collation` and emitted as **`#[index = "..."]`** lines using structured [`IndexKeyPart`](../../src/query/table/definition.rs) / `expr | coverage` for expression keys (expression indexes whose coverage cannot be inferred from table column tokens are **skipped**).
...
**Phase A closure (documentation + tests):** **`infer-schema` CLI subprocess e2e** — `lifeguard-migrate/tests/infer_schema_cli_subprocess.rs` (spawns `CARGO_BIN_EXE_lifeguard-migrate infer-schema`, asserts banner; skips without DB URL). **Library / CI:** `infer_schema_postgres_smoke.rs`, `infer_schema_table_filter_si3.rs` (unchanged). **DBA confidence — live DB vs on-disk generated migrations:** `lifeguard_migrate::schema_migration_compare` + CLI **`compare-schema`** — reconciles **`information_schema` base table names**; for tables present in both baselines, **column names** (`information_schema.columns` vs merged `CREATE TABLE` + `ADD COLUMN` via `column_map_from_merged_baseline`); **simple btree index key + `INCLUDE` column names** from `pg_indexes.indexdef` vs that merged column map when there is **no** migration `CREATE INDEX` line for that index name; **normalized `CREATE INDEX` text** when migration and live share the same index name (**T1**); **indexes only in DB or only in migrations**; **non-btree `USING`** vs assumed btree baseline; and **T2b (partial):** **`pg_index` / `pg_opclass`** non-default btree opclass vs type default on shared tables (**§5.7a**). **Does not** compare SQL type text, constraints, migration-**expected** opclass from entities, full collation/NULLS ordering parity, or expression keys when the parser returns nothing useful. `tests/migration_db_compare_smoke.rs`. **Docs:** `lifeguard-migrate/README.md`, `lifeguard_migrate::schema_migration_compare` rustdocs, `DEVELOPMENT.md` (migrate section). Relation/index appendix: [DESIGN_FIND_RELATED_SCOPES.md](./DESIGN_FIND_RELATED_SCOPES.md).
...
### 5.7a Phase A stretch — **PRD closure** (post-PRD backlog → roadmap)
...
- **~~Index reconciliation (name-level + INCLUDE)~~** — **shipped:** entity-driven SQL (`lifeguard_migrate::sql_generator`, `IndexDefinition`) emits **`CREATE [UNIQUE] INDEX … (keys) INCLUDE (…) WHERE …`**; `#[index = "…"]` supports **`INCLUDE (cols)`** after the key list. **`compare-schema`** checks key + **INCLUDE** column names vs the merged migration baseline (simple btree / `indexdef` parse; expression indexes still skipped when unparseable).
- **~~Non-btree index access method (roadmap T2 baseline)~~** — **shipped:** `lifeguard_migrate::schema_migration_compare` reports live indexes whose **`USING`** is not **`btree`** (implicit or explicit); **`IndexAccessMethodDrift`** — entity migrations assume btree-oriented indexes.
- **~~Normalized index statement text + presence (roadmap T1)~~** — **shipped:** `normalize_index_statement_for_compare`, **`IndexDefinitionTextDrift`**, **`IndexOnlyInDatabaseDrift`**, **`IndexOnlyInMigrationDrift`** (see `schema_migration_compare`, `generated_migration_diff`).
- **~~Derive field ↔ index coverage (roadmap T4)~~** — **shipped (opt-in):** **`#[require_index_coverage]`** on **`LifeModel`** — `validate_require_index_coverage` in `lifeguard-derive`.
- **~~Btree opclass vs type default (roadmap T2b, catalog)~~** — **shipped (partial):** **`fetch_live_btree_index_key_opclasses`**, **`IndexBtreeNonDefaultOpclassDrift`** — `pg_opclass.opcdefault` for simple btree key columns (PostgreSQL 12+); expression key slots skipped.

**Post-PRD (optional):** **T2b** follow-ons (expected opclass from migration/entity, **T1** dedupe), richer expression-key parity (**T3**) — **[DESIGN_INDEX_COMPARE_ROADMAP.md](./DESIGN_INDEX_COMPARE_ROADMAP.md)**, **[ROADMAP.md](../../ROADMAP.md)**.
...
**PRD Phase C optional codegen — closed for this PRD:** declarative **named** scope lists on the **struct** (beyond **`#[scope_bundle]`**) are **out of scope** here; use **`#[scope_bundle]`** on `impl Entity` to AND-combine **`#[scope]`** helpers. Further sugar is **roadmap-only** if filed. **Implicit** merge of parent `SelectQuery` into loaders / multi-hop joins: **not** implemented; **[DESIGN_INHERITED_PARENT_SCOPES_SPIKE.md](./DESIGN_INHERITED_PARENT_SCOPES_SPIKE.md)** (**A** + **D**). Appendix: [DESIGN_FIND_RELATED_SCOPES.md](./DESIGN_FIND_RELATED_SCOPES.md) §C. **Done in-tree:** `SelectQuery::scope_or` / `scope_any` (PRD SC-2); **`#[scope]`** (`lifeguard::scope`) on `impl Entity` renames `fn foo` → `scope_foo`; **`#[scope_bundle]`** (`lifeguard::scope_bundle`) AND-combines existing scopes into `scope_<name>() -> sea_query::Condition` (tests: `lifeguard-derive/tests/test_minimal.rs` `scope_bundle_and_chains`); **`FindRelated::find_related_scoped`** — related-side scope in one call (`tests/db_integration/related_trait.rs` `test_find_related_scoped_matches_chained_scope`); **`FindRelated::find_related_parent_scoped`** — opt-in **`INNER JOIN`** on `RelationDef::from_tbl` + `IntoCondition` on **`Self::Entity`** (caller-side / parent-table predicates; `has_many_through` not supported yet; `tests/db_integration/related_trait.rs` `test_find_related_parent_scoped_joins_from_table`). **`find_related` vs scopes:** default behavior documented in crate rustdoc (`query::scope`, `FindRelated`) and [DESIGN_FIND_RELATED_SCOPES.md](./DESIGN_FIND_RELATED_SCOPES.md) (parent scopes are not merged by default; chain on the returned query or use `find_related_scoped` / `find_related_parent_scoped`). README matrix (G6) updated for scopes.
...
**PRD Phase E mapping — closed for this PRD cycle:** keep **[SEAORM_LIFEGUARD_MAPPING.md](./lifeguard-derive/SEAORM_LIFEGUARD_MAPPING.md)** session / UoW snapshot aligned with **`src/session/`** via **G6** ([DEVELOPMENT.md](../DEVELOPMENT.md)); snapshot updated in this closure. **Done in-tree:** **`LifeRecord::attach_session_with_model`** — linked `Rc<RefCell<Model>>` updated via **`to_model()`** on each session-notifying mutation when conversion succeeds (PRD §9 / `DESIGN_SESSION_UOW.md`).
