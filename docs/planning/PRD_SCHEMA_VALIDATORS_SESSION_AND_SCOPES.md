...
- **Tests:** `src/session/mod.rs`, `src/session/pk.rs` — identity map, fingerprint, dirty order, flush error retention. **`db_integration_suite`:** `tests/db_integration/session_identity_flush.rs` — `register_loaded` → mutate `Rc` → `mark_dirty` / `mark_dirty_key` + `identity_map_key` → `flush_dirty` → `LifeRecord::update` on Postgres.
- **Process:** `docs/planning/DEV_RUSTDOC_AND_COVERAGE.md` and `DEVELOPMENT.md` (rustdoc + coverage checklist for feature work).
