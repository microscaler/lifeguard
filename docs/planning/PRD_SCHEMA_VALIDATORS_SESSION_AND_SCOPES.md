...
**Phase A closure (documentation + tests):** **`infer-schema` CLI subprocess e2e** — `lifeguard-migrate/tests/infer_schema_cli_subprocess.rs` (spawns `CARGO_BIN_EXE_lifeguard-migrate infer-schema`, asserts banner; skips without DB URL). **Library / CI:** `infer_schema_postgres_smoke.rs`, `infer_schema_table_filter_si3.rs` (unchanged). **DBA confidence — live DB vs on-disk generated migrations:** `lifeguard_migrate::schema_migration_compare` + CLI **`compare-schema`** — reconciles **`information_schema` base table names**; for tables present in both baselines, **column names** (`information_schema.columns` vs merged `CREATE TABLE` + `ADD COLUMN` via `column_map_from_merged_baseline`); and **simple btree index key + `INCLUDE` column names** from `pg_indexes.indexdef` vs that merged column map (primary key indexes skipped; expression indexes skipped when unparseable). **Does not** compare SQL type text, constraints, **operator classes** / full `CREATE INDEX` text equality, or partial-index predicate semantics beyond what the parser extracts. `tests/migration_db_compare_smoke.rs`. **Docs:** `lifeguard-migrate/README.md` (`infer-schema`, `compare-schema` including **limits and roadmap**), `lifeguard_migrate::schema_migration_compare` rustdocs, `DEVELOPMENT.md` (migrate section). Deferred index / relation semantics: [DESIGN_FIND_RELATED_SCOPES.md](./DESIGN_FIND_RELATED_SCOPES.md) appendix.
...
- **~~Index reconciliation (remainder)~~ — mostly shipped:** entity-driven SQL (`lifeguard_migrate::sql_generator`, `IndexDefinition`) emits **`CREATE [UNIQUE] INDEX … (keys) INCLUDE (…) WHERE …`**; `#[index = "…"]` supports **`INCLUDE (cols)`** after the key list. **`compare-schema`** checks key + **INCLUDE** column names vs the merged migration baseline (simple btree / `indexdef` parse; expression indexes still skipped). **Still deferred:** opclass / operator-class parity, full **`CREATE INDEX`** text equality vs live `pg_indexes`, optional derive-time “field ↔ indexed column” checks — phased in **[DESIGN_INDEX_COMPARE_ROADMAP.md](./DESIGN_INDEX_COMPARE_ROADMAP.md)**.
...
**Still to do for fuller Phase E:** **[SEAORM_LIFEGUARD_MAPPING.md](./lifeguard-derive/SEAORM_LIFEGUARD_MAPPING.md)** session / UoW matrix rows as APIs grow (**ongoing**; batch with feature PRs or doc-only PRs — same discipline as **G6** in [DEVELOPMENT.md](../DEVELOPMENT.md)). **Done in-tree:** **`LifeRecord::attach_session_with_model`** — linked `Rc<RefCell<Model>>` updated via **`to_model()`** on each session-notifying mutation when conversion succeeds (PRD §9 / `DESIGN_SESSION_UOW.md`).
