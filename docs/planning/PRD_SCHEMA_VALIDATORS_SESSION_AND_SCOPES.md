...
- [x] **Phase E — Session / Unit of Work (v0 — identity map + session handle)** ([§9](#9-session--unit-of-work-identity-map-dirty-tracking)) — **v0:** `ModelIdentityMap`, `Session`, `SessionDirtyNotifier`, `attach_session` / record auto-dirty enqueue; **deferred:** pool pin-slot UoW, insert-only flush; see [§9.7](#97-implementation-status-v0--u-2-partial)
...
- **Tests:** `src/session/mod.rs`, `src/session/pk.rs`, `src/session/uow.rs` — identity map, fingerprint, dirty order, flush error retention, `Session` pending merge, `SessionDirtyNotifier` `Send`. **`db_integration_suite`:** `tests/db_integration/session_identity_flush.rs` — raw map flush, `mark_dirty_key` + `identity_map_key`, **`Session` + `attach_session` + record `set_*`** (model synced before flush) → `LifeRecord::update` on Postgres.
- **Process:** `docs/planning/DEV_RUSTDOC_AND_COVERAGE.md` and `DEVELOPMENT.md` (rustdoc + coverage checklist for feature work).

**Still to do for fuller Phase E:** optional pool **pin-slot** UoW (U-4), insert-only flush for new rows, auto-sync **Model** from **LifeRecord** (vs app-maintained consistency), mapping matrix row tweaks as APIs grow.
