...
|------------|------------------------|--------|-------|
| **Schema inference (DB → Rust)** | `lifeguard-migrate infer-schema`, `compare-schema` (tables + column names + index key / **INCLUDE** names + **non-btree `USING`** drift vs assumed btree baseline), `schema_infer::emit_inferred_rust`, `schema_migration_compare`, `generated_migration_diff::column_map_from_merged_baseline`, `tests/golden/*.expected.rs`, `just bless-infer-schema-goldens` | 🟡 **Partial** | PRD §5.7 / §5.7a **closed** for this scope; not full `indexdef` text / btree opclass tokens — **[DESIGN_INDEX_COMPARE_ROADMAP.md](../DESIGN_INDEX_COMPARE_ROADMAP.md)** |
...
| **Scopes** | `SelectQuery::scope`, `scope_or`, `scope_any`, `IntoScope`, `#[scope]` / **`#[scope_bundle]`** on `impl Entity` (`lifeguard::scope`, `lifeguard::scope_bundle`), **`FindRelated::find_related_scoped`**, **`FindRelated::find_related_parent_scoped`** (opt-in `INNER JOIN` on `from_tbl` + caller-side `IntoCondition`), `src/query/scope.rs` | 🟡 **Partial** | PRD §7.7; AND + OR composition; **`scope_bundle`** ANDs named `#[scope]` helpers into one `scope_*() -> Condition`; **`find_related_scoped`** = `find_related()?.scope(…)` on the **related** table; **`find_related_parent_scoped`** merges **caller-side** table predicates via join; soft-delete interaction; default **`find_related`** does not merge parent scopes — see [DESIGN_FIND_RELATED_SCOPES.md](../DESIGN_FIND_RELATED_SCOPES.md), rustdoc on `query::scope` / `FindRelated`, **`tests/db_integration/related_trait.rs`**, **`lifeguard-derive/tests/test_minimal.rs`** (`scope_bundle_and_chains`) |
...
| **Session / UoW** | `ModelIdentityMap`, `fingerprint_pk_values`, `mark_dirty` / `mark_dirty_key` / `flush_dirty` / `flush_dirty_with_map_key`, `register_pending_insert` / `promote_pending_to_loaded` / `is_pending_insert_key`, `Session`, `LifeRecord::identity_map_key`, **`LifeRecord::attach_session` / `attach_session_with_model`**, **`Session::flush_dirty_in_transaction`** / **`flush_dirty_in_transaction_pooled`**, **`LifeguardPool::exclusive_primary_write_executor`**, `src/session/` | 🟡 **Partial** | PRD §9.7 **mapping closure:** keep this row aligned with `src/session/` (**G6**); not full SeaORM session semantics |
