...
| **Scopes** | `SelectQuery::scope`, `scope_or`, `scope_any`, `IntoScope`, `#[scope]` / **`#[scope_bundle]`** on `impl Entity` (`lifeguard::scope`, `lifeguard::scope_bundle`), **`FindRelated::find_related_scoped`**, `src/query/scope.rs` | 🟡 **Partial** | PRD §7.7; AND + OR composition; **`scope_bundle`** ANDs named `#[scope]` helpers into one `scope_*() -> Condition`; **`find_related_scoped`** = `find_related()?.scope(…)`; soft-delete interaction; **`find_related` does not inherit parent `scope` predicates** — chain `.scope`/`.filter` or `find_related_scoped` on related rows; see [DESIGN_FIND_RELATED_SCOPES.md](../DESIGN_FIND_RELATED_SCOPES.md), rustdoc on `query::scope` / `FindRelated`, **`tests/db_integration/related_trait.rs`**, **`lifeguard-derive/tests/test_minimal.rs`** (`scope_bundle_and_chains`) |
