...
|------------|------------------------|--------|-------|
| **Schema inference (DB → Rust)** | `lifeguard-migrate infer-schema`, `compare-schema` (live tables vs merged `*_generated_from_entities.sql`), `schema_infer::emit_inferred_rust`, `schema_migration_compare`, `tests/golden/*.expected.rs` | 🟡 **Partial** | PRD §5.7; deterministic emitter golden tests; conservative type mapping; composite PK gaps possible; `compare-schema` is table-name-only reconciliation (not column diff) |
