...
|------------|------------------------|--------|-------|
| **Schema inference (DB → Rust)** | `lifeguard-migrate infer-schema`, `compare-schema` (tables + column names + index key / **INCLUDE** column names vs merged SQL where parseable), `schema_infer::emit_inferred_rust`, `schema_migration_compare`, `generated_migration_diff::column_map_from_merged_baseline`, `tests/golden/*.expected.rs`, `just bless-infer-schema-goldens` | 🟡 **Partial** | PRD §5.7 / §5.7a; deterministic emitter goldens; `infer-schema --watch`; `compare-schema` is name-level (not full `CREATE INDEX` / opclass parity) |
