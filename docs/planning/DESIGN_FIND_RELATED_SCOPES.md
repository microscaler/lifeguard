...
**Compared (partial):** **Access method** — live indexes must use **`btree`** (implicit or explicit); **`USING gin` / `gist` / `hash` / …** is reported as drift (`lifeguard_migrate::schema_migration_compare`). **T2b (catalog):** per **simple** btree key column, if the live opclass is not the type’s default (`pg_opclass` / `opcdefault`), drift is reported (e.g. **`text_pattern_ops`** vs default **`text_ops`** on **`text`** — note **`jsonb_path_ops` is GIN-only**, not a btree opclass). **Still not modeled:** per-column **collation**, **NULLS FIRST/LAST**, **expression** key slots (skipped), or asserting migration **expected** opclass from entity IR — two btree indexes can still differ only in those dimensions while sharing the same column names, so **`compare-schema` may not report drift** for those gaps.
