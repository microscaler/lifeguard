...
### Phase 4 — Proc-macro hygiene
- [x] **P4.1** — Relation macro: `panic!` / infallible `expect` → `syn::Error` / `Option<RelationDef>` for `def()` where feasible (see §12).
- [x] **P4.2** — Tuple-struct field names: `field.ident` `unwrap()` → `utils::field_ident` → `syn::Error`; emitted `graph_mut` uses `get_or_insert_with` (no `unwrap`); `column_type` branch uses `if let Some`.
- [x] **P4.3** — `lifeguard-derive` crate root `#![deny(clippy::unwrap_used)]`; scoped `#![allow(clippy::unwrap_used)]` on `type_conversion` tests and `life_model::relation_parse_error_tests`.
...
| P3.2 | same | pass |
| P4.1–P4.3 | `cargo clippy -p lifeguard-derive --all-targets --all-features -- -D warnings`; `cargo test -p lifeguard-derive` | pass |
...
4. **`DeriveRelation` generated `Relation::def()`** returns **`Option<RelationDef>`**. Annotated variants return `Some(...)`; variants without `#[lifeguard(...)]` return `None`. Use `.expect(...)` / `if let Some` at call sites (replacing any assumption of a non-optional `RelationDef`).
...
| File | Pattern | Notes |
|------|---------|--------|
| `life_record.rs` | (resolved P4) `field_ident` + `get_or_insert_with` for `graph_mut` | — |
| `life_model.rs`, `from_row.rs`, `try_into_model.rs`, `partial_model.rs` | (resolved P4) `utils::field_ident` | Tuple / unnamed fields → compile error at expand time. |
| `relation.rs` | (resolved P4) errors via `syn::Error`; `def()` → `Option` | Unannotated variants: `None`. |
...
*PRD updated 2026-03-28 with execution checklists, verification log, and appendices. Phase 4 (proc-macro hygiene) completed.*
