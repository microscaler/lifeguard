...
**Status:** **T1**, **T2**, **T4** shipped; **T2b** **partially** shipped (**catalog** non-default btree opclass vs type default on shared tables — see [`fetch_live_btree_index_key_opclasses`](../lifeguard-migrate/src/schema_migration_compare.rs)). **T3** **partially** shipped (**catalog** expression btree keys vs merged simple-column `CREATE INDEX`; **T1** suppressed for those indexes — see [`fetch_live_btree_expression_index_key_slots`](../lifeguard-migrate/src/schema_migration_compare.rs)). **T2b** follow-ons and **T3** v2+ remain optional; see [ROADMAP.md](../../ROADMAP.md). Complements [`lifeguard-migrate/README.md`](../lifeguard-migrate/README.md).
...
| **T3 — Expression / functional keys** | **Partial:** live expression btree key + migration simple keys → structured drift (**T1** suppressed). **Follow-on:** both-sides expression compare, `IndexDefinition` / derive | Catalog [`fetch_live_btree_expression_index_key_slots`](../lifeguard-migrate/src/schema_migration_compare.rs) (`pg_get_indexdef` per key) + [`parse_pg_indexdef_simple_columns`](../lifeguard-migrate/src/schema_migration_compare.rs) on merged `CREATE INDEX` | High complexity for v2+ |
