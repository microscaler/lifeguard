...
fn generate_partial_model_code(
    ...
    code.push_str(&format!(
        "impl lifeguard::PartialModelTrait for {} {{\n",
        struct_name
    ));
    ...
    for field in fields {
        ...
        let get_expr = match field.ty.as_str() {
            ...
            _ => {
                format!(
                    "            {}: row.try_get::<&str, {}>(\"{}\")?,",
                    field.name, field.ty, column_name
                )
            }
        };
...
mod tests {
    ...
    fn test_mixed_types() {
        ...
        assert!(!code.contains(
            "let val: i64 = row.try_get::<&str, i64>(\"count\")?;\n                val as u128"
        ));
    }
}
