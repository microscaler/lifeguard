...
fn test_empty_registry_structure_valid() {
    ...
    assert!(
        content.contains("pub fn generate_sql_for_all() -> Result<Vec<(String, String)>, String>")
    );
...
fn test_empty_registry_after_discovery_error() {
    ...
    assert!(
        result.is_ok(),
        "Should generate empty registry even when entities list is empty"
    );
...
fn test_missing_registry_file_would_fail() {
    ...
    assert!(
        ...
        "Registry file should not exist (simulating old bug)"
    );
...
fn test_empty_registry_prevents_missing_file_error() {
    ...
    assert!(
        registry_file.exists(),
        "Registry file should exist even with empty entities"
    );
