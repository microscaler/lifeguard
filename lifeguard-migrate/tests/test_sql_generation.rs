...
fn test_chart_of_accounts_sql_generation() {
    ...
    println!(
        "id column_type = {:?} (should be Some(\"UUID\") from type inference)",
        id_def.column_type
    );
    ...
    println!(
        "code column_type = {:?} (should be Some(\"VARCHAR(50)\"))",
        code_def.column_type
    );
    ...
    println!(
        "name column_type = {:?} (should be Some(\"VARCHAR(255)\"))",
    ...
    println!(
        "level default_value = {:?} (should be Some(\"0\"))",
        level_def.default_value
    );
    ...
    println!(
        "parent_id nullable = {} (should be true - it's Option<uuid::Uuid>)",
        parent_id_def.nullable
    );
    ...
    // Generate SQL
    let generated_sql =
        sql_generator::generate_create_table_sql::<Entity>(table_def).expect("Should generate SQL");
    ...
    if !original_file.exists() {
        eprintln!(
            "Warning: Original migration file not found: {:?}",
            original_file
        );
    ...
    let original_sql = fs::read_to_string(&original_file).expect("Should read original SQL file");

    // Extract chart_of_accounts table definition from original
    let original_table_sql =
        extract_table_sql(&original_sql, "chart_of_accounts").expect("Should extract table SQL");
