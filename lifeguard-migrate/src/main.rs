...
fn handle_compare_schema(
    ...
    if report.has_drift() {
        return Err(MigrationError::InvalidFormat(
            "compare-schema: live database does not match merged generated migration baseline (tables, column names, index keys, index definitions, index presence, and/or index access methods — see above)."
                .to_string(),
        ));
    }
    Ok(())
