...
use lifeguard::migration::{MigrationError, Migrator};
        ...
        _ => {
            ...
            match cli.command {
                ...
                Commands::Down { steps, dry_run } => {
                    handle_down(&migrator, &executor, steps, dry_run)
                }
...
fn handle_compare_schema(
    ...
    if report.has_drift() {
        return Err(MigrationError::InvalidFormat(
            "compare-schema: live database does not match merged generated migration baseline (tables, column names, index keys, index definitions, index presence, btree operator classes, expression vs simple index keys, and/or index access methods — see above)."
                .to_string(),
        ));
    }
    Ok(())
...
fn handle_generate(migrations_dir: &PathBuf, name: &str) -> Result<(), MigrationError> {
    ...
    fs::create_dir_all(migrations_dir).map_err(|e| {
        MigrationError::FileNotFound(format!("Failed to create migrations directory: {}", e))
    })?;
...
"#,
    ...
    file.write_all(template.as_bytes()).map_err(|e| {
        MigrationError::FileNotFound(format!("Failed to write migration file: {}", e))
    })?;

    file.sync_all().map_err(|e| {
        MigrationError::FileNotFound(format!("Failed to sync migration file: {}", e))
    })?;
...
fn handle_generate_from_entities(
    ...
    println!(
        "⚠️  WARNING: Using file-based discovery - migrations may not match compiled entities!"
    );
    ...
    let mut entities_by_service: HashMap<Option<String>, Vec<&entity_loader::EntityInfo>> =
        HashMap::new();
    ...
    for (service_path, service_entities) in entities_by_service {
        ...
        println!(
            "   ⚠️  Found {} entities in service {:?}",
            service_entities.len(),
            service_path
        );
        for entity_info in service_entities {
            println!(
                "   ⚠️  Would generate SQL for {} (registry implementation pending)",
...
fn handle_generate_from_registry(output_dir: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_handle_generate_creates_file() {
        ...
        assert!(
            result.is_ok(),
            "Should successfully generate migration file"
        );
        ...
        assert!(
            files[0].to_string_lossy().starts_with("m"),
            "File should start with 'm'"
        );
        assert!(
            files[0].to_string_lossy().contains(name),
            "File should contain migration name"
        );
        assert!(
            files[0].to_string_lossy().ends_with(".rs"),
    ...
    fn test_handle_generate_prevents_overwrite_existing_file() {
        ...
        assert!(
            result2.is_ok(),
            "Second generation with different timestamp should succeed"
        );
        ...
        assert_eq!(
            original_content, still_original_content,
    ...
    fn test_handle_generate_fails_when_file_exists() {
        ...
        if result.is_err() {
            ...
            assert_eq!(
                files.len(),
                2,
                "Should have exactly 2 files (original + new)"
            );
        }
    }
    ...
    fn test_handle_generate_atomic_file_creation() {
        ...
        assert!(
            content.contains("impl Migration for Migration"),
            "Should contain Migration trait impl"
        );
    }
