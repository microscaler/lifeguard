...
pub fn generate_registry_module(
        ...
        let service_path = entity.file_path
            ...
            .and_then(|p| {
                ...
                if let Some(src_idx) = parts.iter().position(|c| {
                    ...
                    parts.drain(..=src_idx);
                    Some(parts.iter().map(|c| c.to_string_lossy().to_string()).collect::<Vec<_>>().join("/"))
                } else {
                    None
                }
            })
    ...
    registry_content.push_str("use lifeguard::LifeModelTrait;\n\n");
    ...
    for (service_path, service_entities) in &entities_by_service {
        ...
        let service_module = service_path.replace("/", "_").replace("-", "_");
        ...
        for entity in service_entities {
            ...
            // Calculate relative path from output to entity file
            // This is complex - for now, use absolute path or path from crate root
            let entity_path = &entity.file_path;
    ...
    // Generate all_entities() function
    registry_content.push_str("    /// Get all registered entities\n");
    registry_content.push_str("    pub fn all_entities() -> Vec<Box<dyn LifeModelTrait>> {\n");
