    ...
    E::Column: ColumnTrait + Copy + sea_query::IdenStatic + PartialEq,
{
    ...
    for col in columns {
        ...
        // Add auto-increment (PostgreSQL uses SERIAL or GENERATED)
        // For UUID, use gen_random_uuid() instead
        if col_def.auto_increment {
            if col_def.column_type.as_ref().map(|s| s.contains("UUID")).unwrap_or(false) {
                if col_def.default_expr.is_none() && col_def.default_value.is_none() {
        ...
        if let Some(ref fk) = col_def.foreign_key {
            foreign_keys.push((col_name.to_string(), fk.clone()));
        }
        
        // Track CHECK constraints (column-level)
        if let Some(ref check) = col_def.check {
            check_constraints.push((col_name.to_string(), check.clone()));
        }
