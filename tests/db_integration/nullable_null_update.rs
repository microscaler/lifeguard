...
//! touched, so `None` was ambiguous: "never touched" and "set me to NULL"
//! looked identical. `update()` therefore emitted no SET clause for
//! `set_x(None)` and the **old value survived** — while the call reported
//! success. Found in the wild twice: a Sesame credential-revocation path that
//! believed it had destroyed a stored secret, and a hauliage flow clearing an
//! expiry after verification.
//!
//! `__null_columns` disambiguates the two. These tests cover both directions:
//! a staged NULL must actually be written, and an untouched column must
//! still be left alone — the fix must not turn every unset `Option` into a
//! `NULL`, which would be a far worse bug than the one it replaces.
...
/// The real-world shape: read the row, clear one field, write it back.
#[test]
