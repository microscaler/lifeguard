...
fn test_find_related_has_many_relationship() {
    ...
    let _post1 = post1_record
        .insert(&executor)
        .expect("Failed to insert post1");
    ...
    let _post2 = post2_record
        .insert(&executor)
        .expect("Failed to insert post2");
    ...
    let other_user = other_user_record
        .insert(&executor)
    ...
    let _post3 = post3_record
        .insert(&executor)
        .expect("Failed to insert post3");
    ...
    assert!(
        titles.contains(&"Second Post"),
        "Should contain second post"
    );
    assert!(
        !titles.contains(&"Other User's Post"),
        "Should not contain other user's post"
    );
}
...
fn test_find_related_empty_result() {
    ...
    assert_eq!(
        posts.len(),
        0,
        "Should find no posts for user with no posts"
    );
}
...
fn test_find_related_multiple_users() {
    ...
    assert_eq!(
        user1_posts[0].user_id, user1.id,
        "Post should belong to user1"
    );
    ...
    assert_eq!(
        user2_posts[0].user_id, user2.id,
        "Post should belong to user2"
    );
}
    ...
    fn to() -> RelationDef {
        RelationDef {
            ...
            from_tbl: TableRef::Table(
                TableName(None, "test_resources_composite".into_iden()),
                None,
            ),
...
fn setup_composite_test_schema(
...
fn cleanup_composite_test_data(
    executor: &MayPostgresExecutor,
) -> Result<(), lifeguard::executor::LifeError> {
...
fn test_find_related_composite_key() {
    ...
    let _resource1 = resource1_record
        .insert(&executor)
        .expect("Failed to insert resource1");
    ...
    let _resource2 = resource2_record
        .insert(&executor)
        .expect("Failed to insert resource2");
    ...
    let _resource3 = resource3_record
        .insert(&executor)
        .expect("Failed to insert resource3");

    // Find resources for the first tenant (should find 2 resources)
    let resources = tenant
        .find_related::<ResourceEntity>()
        .expect("find_related")
        .all(&executor)
    ...
    for resource in &resources {
        assert_eq!(
            ...
            "Resource should belong to the tenant"
        );
        assert_eq!(
            resource.region_id, tenant.region_id,
            "Resource should be in the same region"
        );
    }
    ...
    assert!(
        !names.contains(&"Other Resource"),
        "Should not contain other tenant's resource"
    );
}
...
fn test_find_related_composite_key_empty() {
    ...
    assert_eq!(
        resources.len(),
        0,
        "Should find no resources for tenant with no resources"
    );
}
...
fn test_composite_key_identity_values_match() {
    ...
    assert_eq!(
        values.len(),
        identity.arity(),
        "Values count should match Identity arity"
    );
}
...
fn test_find_related_belongs_to_relationship() {
    ...
    assert_eq!(
        users[0].id, post.user_id,
        "User ID should match post's user_id foreign key"
    );
